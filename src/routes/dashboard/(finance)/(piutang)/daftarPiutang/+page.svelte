<script lang="ts">
  ...
  const tableData = writable(data.piutangData);
  $: tableData.set(data.piutangData);
  ...
  const form = superForm(data.form, {
    ...
    async onUpdate({ form }) {
      if (form.valid) {
        ...
        await goto('/dashboard/daftarPiutang');
      }
    },
    onError(event) {
  ...
  $: $formData.total = $formData.piutang.reduce((a, i) => a + Number(i.nilai), 0);
...
<div class="flex flex-col gap-4">
  <Breadcrumb.Root>
    <Breadcrumb.List>
      ...
      <Breadcrumb.Item>
        <Breadcrumb.Page>Piutang</Breadcrumb.Page>
      </Breadcrumb.Item>
    </Breadcrumb.List>
  </Breadcrumb.Root>
  <div class="flex items-center justify-between">
    <div class="flex flex-col gap-1">
      <h1 class="text-3xl font-bold">Piutang</h1>
    </div>
  </div>
